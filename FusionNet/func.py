...
def fuse_vecotr(howA, howB, q_mask, attention_dim, keep_prob, is_train, scope="fusion", B=None):
    # self or match attetion
    B = B if B is not None else howB
    "Fuses glove representations of question into para"
    # Sij=f(U*howA)^T*D*f(U*(howB))
    with tf.variable_scope(scope):
        d_howA = dropout(howA, keep_prob=keep_prob, is_train=is_train)
        d_howB = dropout(howB, keep_prob=keep_prob, is_train=is_train)
        JX = tf.shape(howA)[1]  # max_len changed
        # -----------------------------------------------------fusion variables
        ...
        with tf.variable_scope("attention"):
            # 线性变换并对齐
            howA_ = tf.nn.relu(
                dense(d_howA, attention_dim, use_bias=False, scope="howA"))
            howB_ = tf.nn.relu(
                dense(d_howB, attention_dim, use_bias=False, scope="howB"))
            med = tf.matmul(D, tf.transpose(howB_, [0, 2, 1]))  # (batch_size,hidden,JX)
            # 计算每一项对应的内积
            outputs = tf.matmul(howA_, med) / (attention_dim ** 0.5) #(batch_size,hidden,hidden)
            mask = tf.tile(tf.expand_dims(q_mask, axis=1), [1, JX, 1])
            # softmax_mask将query的mask位为零的位置，置为-INF，softmax后为零
            logits = tf.nn.softmax(softmax_mask(outputs, mask))
            outputs = tf.matmul(logits, B)
            return outputs
            #gate attention
        #     res = tf.concat([howA, outputs], axis=2)
        #     # gate attention
        # with tf.variable_scope("gate"):
        #     dim = res.get_shape().as_list()[-1]
        #     d_res = dropout(res, keep_prob=keep_prob, is_train=is_train)
        #     gate = tf.nn.sigmoid(dense(d_res, dim, use_bias=False))
        #     return res * gate
def word_fusion_vecotr(inputs, memory, q_mask, keep_prob,is_train, scope="word_fusion"):
    "Fuses glove representations of question into para"
    hidden = memory.get_shape().as_list()[-1]
    with tf.variable_scope(scope):
        d_inputs = dropout(inputs, keep_prob=keep_prob, is_train=is_train)
        d_memory = dropout(memory, keep_prob=keep_prob, is_train=is_train)
        JX = tf.shape(inputs)[1]
...
def summ_vector(memory, hidden, mask, keep_prob=1.0, is_train=None, scope="summ"):
    with tf.variable_scope(scope):
        d_memory = dropout(memory, keep_prob=keep_prob, is_train=is_train)
        s0 = tf.nn.tanh(dense(d_memory, hidden, scope="s0"))
        s = dense(s0, 1, use_bias=False, scope="s")
