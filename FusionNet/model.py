...
class Model(object):
    def __init__(self, config, batch, word_mat=None, char_mat=None, trainable=True, opt=False):
        self.config = config
        ...
        self.c_mask = tf.cast(self.c, tf.float32)
        self.q_mask = tf.cast(self.q, tf.float32)
        self.c_mask=tf.reshape(self.c_mask,[N,config.para_limit])
        self.q_mask=tf.reshape(self.q_mask,[N,config.ques_limit])
        self.c_mask=tf.expand_dims(self.c_mask,axis=2)
        self.q_mask=tf.expand_dims(self.q_mask,axis=2)
        self.c_len = tf.reduce_sum(tf.cast(self.c_mask, tf.int32), axis=1)
        self.q_len = tf.reduce_sum(tf.cast(self.q_mask, tf.int32), axis=1)
        self.c_len=tf.squeeze(self.c_len)
        self.q_len=tf.squeeze(self.q_len)
        if opt:
            N = config.batch_size
