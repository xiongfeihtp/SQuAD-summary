...
def process_file(raw_data, data_type, word_counter, char_counter, config, feature_dict):
    ...
    for data_item in tqdm(raw_data):
        ...
        #这里的feature的行是没有对齐的, len(context_tokens),len(dict_feature)

        # 生成examle格式字典
        example = {"feature": feature, "context_tokens": context_tokens, "context_chars": context_chars,
...
def build_features(config, examples, data_type, out_file, word2idx_dict, char2idx_dict, is_test=False):
    ...
    for example in tqdm(examples):
        ...
        record = tf.train.Example(features=tf.train.Features(feature={
            ...
            "feature": tf.train.Feature(bytes_list=tf.train.BytesList(values=[example["feature"].tostring()]))
        }))
...
def prepro(config):
    ...
    raw_data_all = raw_data_train.extend(raw_data_dev)
    feature_dict = build_feature_dict(config, raw_data_all)

    train_examples, train_eval = process_file(
        raw_data_train, "train", word_counter, char_counter, config, feature_dict)
    dev_examples, dev_eval = process_file(
        raw_data_dev, "dev", word_counter, char_counter, config, feature_dict)
    test_examples, test_eval = process_file(
        raw_data_test, "test", word_counter, char_counter, config, feature_dict)
    ...
    save(config.char2idx_dict_file, char2idx_dict, message="word2idx_dict")
    save(config.word_counter_file, dict(word_counter), message="word_counter")
    save(config.char_counter_file, dict(char_counter), message="char_counter")
